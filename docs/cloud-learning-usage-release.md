...
- evaluation records contain a text-free rating/action plus bounded role, relationship-stage, and goal-category fields;
...
Save improvement uploads only a text-free rating or a separately authored, attested, sanitized target. It never uploads raw prompts, raw replies, provider reasoning, original drafts, contact IDs, contact names, companies, profile URLs, email addresses, phone numbers, or other known direct identifiers. Known values are replaced before preview, residual direct identifiers are rejected, and the exact sanitized target must be reviewed before both attestations are accepted.
...
4. **Approved record creation.** Save a text-free rating. Separately open the blank independent-author flow, write a synthetic target from scratch, verify known-name/company/profile replacement, verify that email/URL/phone input is rejected, review the exact sanitized preview, and complete both attestations. Confirm one explicit human stage confirmation produces only bounded classifier features and no message text.
