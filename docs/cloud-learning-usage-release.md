...
Copy, Useful, and Not useful store only a bounded text-free evaluation after a successful clipboard write. Each generated draft uses one mutable learning record. Add my own version starts blank, sanitizes and previews the independently authored sanitized target, requires both rights and privacy attestations, and atomically replaces the same Not useful decision row. The direct decision flow never uploads raw prompts, raw replies, provider reasoning, original drafts, contact IDs, contact names, companies, profile URLs, email addresses, phone numbers, or other known direct identifiers. Known values are replaced before preview, residual direct identifiers are rejected, and the exact sanitized target must be reviewed before both attestations are accepted.
