...
await cp(join(root, "node_modules", "tesseract.js-core"), join(root, "public", "tesseract-core"), {
  recursive: true,
  // pnpm exposes package roots as directory junctions on Windows. Dereference
  // the junction so the self-hosted OCR assets are copied like an npm install.
  dereference: true,
  filter: (source) => /(?:tesseract-core.*\.(?:js|wasm)|tesseract.js-core)$/.test(source),
});
