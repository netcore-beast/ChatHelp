...
- **Approved learning:** Approved learning records are server-readable, de-identified, purpose-limited, and retained for 365 days. Copy, Useful, and Not useful create only bounded text-free evaluations after a successful clipboard write. Each generated draft has one mutable learning record. Add my own version starts blank, sanitizes and previews the independently authored sanitized target, requires both rights and privacy attestations, and atomically replaces the same Not useful decision row. Classifier records contain bounded structured features. Raw prompts, generated replies, provider reasoning, original drafts, contact IDs, names, companies, URLs, and other direct identifiers are prohibited.
