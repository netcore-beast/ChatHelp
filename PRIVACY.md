...
Copy stores a bounded text-free Useful evaluation only after a successful clipboard write. Useful and Not useful directly store bounded text-free evaluations. Each generated draft has one mutable learning record. Add my own version starts with a blank editor, sanitizes and previews the independently authored sanitized target, requires both rights and privacy attestations, and atomically replaces the same Not useful decision row. These direct actions never upload raw prompts, raw replies, provider reasoning, original drafts, contact IDs, contact names, companies, profile URLs, email addresses, phone numbers, or other direct identifiers. Known values are replaced before the exact preview, and residual identifier patterns are rejected. Individual records can be deleted. Disable-and-delete disables learning and removes every approved learning record for the signed-in account.
